...
volatile uint32_t TFA433::isrCount = 0;

void tfa433_isr_trampoline() {
  TFA433::isrCount++;    // count every interrupt
  //Serial.println(TFA433::isrCount);
    TFA433::callHandler(); // process the pulse
}

void TFA433::callHandler() {
    _handler();
}
...
void TFA433::start(int pin) {
	...
	attachInterrupt(digitalPinToInterrupt(_pin), tfa433_isr_trampoline, CHANGE);
}
